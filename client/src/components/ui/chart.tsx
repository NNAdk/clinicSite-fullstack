...
type ValueType = any;
type Payload = any;
type TooltipProps<V = any, N = any> = any;
...
>(
  (
    ...
    const hasPayload = typedPayload && typedPayload.length > 0;
    ...
    const tooltipLabel = React.useMemo(() => {
      ...
      const value = labelFormatter
        ? labelFormatter(label, typedPayload!)
        : (typeof label === 'string' ? config[label as keyof typeof config]?.label || label : itemConfig?.label || label);
      ...
      return null;
    }, [label, labelFormatter, typedPayload, hideLabel, labelClassName, config, labelKey, hasPayload]);
...
function getPayloadConfigFromPayload(config: ChartConfig, payload: unknown, key: string) {
  ...
  const payloadPayload =
    'payload' in typedPayload && typeof typedPayload.payload === 'object' && typedPayload.payload !== null ? typedPayload.payload : undefined;

  let configLabelKey: string = key;

  if ('dataKey' in typedPayload && typeof typedPayload.dataKey === 'string' && typedPayload.dataKey in config) {
    configLabelKey = typedPayload.dataKey;
  }
  else if (key in typedPayload && typeof typedPayload[key as keyof typeof typedPayload] === 'string') {
